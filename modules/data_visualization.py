...
def visualize_percentage_distribution(df: pd.DataFrame):
    ...
    df_category = df.groupby("Category")["Amount"].agg(Amount='sum').reset_index()
    df_category = df_category.sort_values(by='Amount', ascending=False)
    df_category = df_category[df_category["Category"] != 'Income']
